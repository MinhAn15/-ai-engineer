...
export default function DocumentDetail() {
    ...
    const fetchDocument = async () => {
        try {
            ...
            if (docRes.data.status === 'completed') {
                ...
                // Fetch segments for coordinate mapping
                try {
                    const segRes = await ocrAPI.getSegments(id);
                ...
                // Fetch image
                try {
                    const imgUrl = await ocrAPI.getImage(id);
                    setImageUrl(imgUrl);
                } catch (e) { }
            }
        } catch (error) {
    ...
    // Calculate highlight box style based on hovered segment
    ...
    // Get label style for the highlight box
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex gap-1 p-1 bg-[#141414] rounded-lg inline-flex">
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-md text-sm font-medium transition ${activeTab === 'segments'
                                ...
                                {activeTab === 'segments' ? (
                                    <div className="space-y-3">
                                        {segments.map((segment) => (
                                            <div
                                                ...
                                                className={`p-4 rounded-lg border transition cursor-pointer ${hoveredSegmentIndex === segment.index
                                            ...
                                            >
                                                ...
                                                <div className="flex items-center gap-2 mb-2">
                                                    <span className={`px-2 py-0.5 rounded text-xs font-bold ${hoveredSegmentIndex === segment.index
                                    ...
                                    <div className="space-y-3">
                                        {extractedFields.length > 0 ? (
                                            <>
                                                {/* Extracted Fields with References */}
                                                {extractedFields.map((field, idx) => (
                                                    <div
                                                        key={idx}
                                                        className="p-4 rounded-lg border border-gray-700 bg-[#1a1a1a]"
                                                        onMouseEnter={() => {
                                                            // Highlight first referenced segment
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <PipelineStep
                                            ...
                                            details={usage.pipeline.ocr.confidence
                                                ? `${(usage.pipeline.ocr.confidence * 100).toFixed(0)}% • ${usage.pipeline.ocr.text_blocks} blocks`
                                                : null
                                            }
                                        />
                                        <div className="flex justify-center">
